...
所有可操作组件必须覆盖同一套状态词汇，不允许业务页面自行省略或重定义：

| 状态 | 视觉要求 | 行为要求 |
|------|---------|---------|
| Default | 保持语义色和完整文字对比度 | 可响应触控、键盘和辅助设备 |
| Pressed / Hover | 背景加深或浅色表面增强；允许 `scale(0.97)` | 150ms 内反馈，不改变布局尺寸 |
| Focus | 品牌色边框 + 2px 半透明聚焦环 | 焦点顺序与视觉顺序一致 |
| Disabled | 中性表面 + 可读的禁用文字 | 阻止事件，不使用透明度掩盖全部内容 |
| Loading | 保持原尺寸，显示进度并适度降低强调 | 阻止重复提交，完成后恢复可操作状态 |
| Error | danger 边框 + 错误聚焦环 + 原因文案 | 文案说明原因和下一步，不仅依赖红色 |

### 5.6 Tag 与 Badge

| 元素 | 规格 | 规则 |
|------|------|------|
| Tag | 高 26px，圆角 6px，浅底深字 | 表达分类或状态；固定使用 primary/success/warn/error/info 语义 |
| Plain Tag | 透明背景 + 1px 完整边框 | 用于筛选条件或低强调身份信息 |
| Closable Tag | 视觉高度 26px，关闭触控区 44×44px | 关闭图标必须是独立点击目标 |
| Badge | 最小 18×18px，字号 11px | 仅承载短数字；超过上限显示 `99+` 等截断值 |
| Dot Badge | 8×8px | 只表达“有新内容”，不能表达严重程度 |
