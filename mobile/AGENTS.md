...
## RedHub Mx Project Rules

- RedHub Mobile is an MIT-based fork of Cool Unix; the public component prefix is `mx-`, the runtime alias is `@/.mx`, and internal type names use `Mx`.
- Keep upstream upgrade paths isolated in `build/unix.ts` and the four-file `.cool` build-compatibility shim; do not reintroduce `cl-`, `cool-`, `Cl*`, or `Cool*` names into application or component APIs.
- The product primary color is government crimson `#C41E3A`; use Tailwind tokens (`primary-*`) and `theme.json` instead of hard-coded brand colors.
- Target WeChat Mini Program and native App first. Check UTS nullability and platform compatibility before using browser-only or App-only APIs.
- Register every page in `pages.json`; prefer existing `mx-*` easycom components and keep reusable UI in `uni_modules/mx-ui`.
