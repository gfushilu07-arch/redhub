...
import type { MxFormRule, MxFormValidateError } from "../types";
import { useParent } from "@/.mx";

export class Form {
	public formRef = ref<MxFormComponentPublicInstance | null>(null);
	public disabled: ComputedRef<boolean>;

	constructor() {
		// 获取 mx-form 实例
		if (this.formRef.value == null) {
			const MxForm = useParent<MxFormComponentPublicInstance>("mx-form");

			if (MxForm != null) {
				this.formRef.value = MxForm;
			}
		}
	...
	// 获取所有错误信息
	getErrors = async (): Promise<MxFormValidateError[]> => {
	...
	// 验证整个表单
	validate = (callback: (valid: boolean, errors: MxFormValidateError[]) => void): void => {
...
class FormItem {
	public formItemRef = ref<MxFormItemComponentPublicInstance | null>(null);
	...
	constructor() {
		...
		if (this.formItemRef.value == null) {
			const MxFormItem = useParent<MxFormItemComponentPublicInstance>("mx-form-item");

			if (MxFormItem != null) {
				this.formItemRef.value = MxFormItem;
			}
		}
